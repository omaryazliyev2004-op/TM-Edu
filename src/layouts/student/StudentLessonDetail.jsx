...
function FileChip({ name }) {
  if (!name) return null;
  return (
    <a className="ld-file" href={fileUrl(name)} target="_blank" rel="noreferrer" title={"Yuklab olish"}>
...
export default function StudentLessonDetail() {
  ...
  const lessonTitle = homework?.title || current?.topic || groupName || "Dars";
  ...
  return (
    <>
      <style>{`
        ...
        .ld-tab { padding:16px 0; font-size:15px; font-weight:600; color:#f97316; border-bottom:2px solid #f97316; }
        .ld-ball { font-size:15px; font-weight:600; color:#f97316; }
        ...
        .ld-lesson { background:#faf3ec; border-radius:14px; padding:8px; box-shadow:0 1px 3px rgba(0,0,0,.05); }
        ...
        .ld-lesson:not(.active) { background:#faf3ec; }
        .ld-lesson:not(.active) .ld-lesson-head:hover { background:#f3e8db; }
        .ld-lesson.active .ld-lesson-head { background:#e8b478; }
        ...
        .ld-vid-sub { display:flex; align-items:center; gap:14px; width:100%; text-align:left; border:none; cursor:pointer; background:#f3ddb8; border-radius:11px; padding:15px 16px; font-size:15px; color:#2b2b2b; transition:background .15s; }
        .ld-vid-sub:hover { background:#ecd0a0; }
        .ld-vid-ic { font-size:20px; flex-shrink:0; }
        .ld-vid-sub .fa-circle-play { color:#c9873c; }
        .ld-vid-sub .fa-circle { color:#d9a35f; }
      `}</style>

      <div className="ld-head">
        <button className="ld-back" onClick={() => navigate(-1)} title={"Orqaga"}>
          <i className="fa-solid fa-arrow-left"></i>
        </button>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: "#222", margin: 0 }}>{groupName || "Darslar"}</h1>
      </div>
        ...
        <div className="ld-main" ref={mainRef}>
          ...
          <div className="ld-card">
            <div className="ld-tabs">
              <span className="ld-tab">{"Vazifalar"}</span>
              {result?.grade != null && <span className="ld-ball">{"Ball"}: {result.grade}</span>}
            </div>

            {loading ? (
              <div className="ld-empty">{"Yuklanmoqda..."}</div>
            ) : !homework && !answer ? (
              <div className="ld-empty">{"Uyga vazifa berilmagan"}</div>
            ) : (
              <div className="ld-hw">
                {/* Uyga vazifa */}
                {homework && (
                  <>
                    <div className="ld-hw-head" style={{ justifyContent: "center" }}>
                      <span className="ld-deadline">
                        <i className="fa-solid fa-circle-exclamation"></i>
                        {"Uyga vazifa muddati"}: {deadline(homework.created_at)}
                      </span>
                    </div>
                    <div className="ld-block">
                      <div className="ld-row-between">
                        <span className="ld-section-title">{"Uyga vazifa"}</span>
                        <span className="ld-files-count">{"Fayllar soni"}: {homework.file ? 1 : 0}</span>
                      </div>
                ...
                {!canSubmit && (
                  <div className="ld-block">
                    <div className="ld-row-between">
                      <span className="ld-section-title">{"Mening jo'natmalarim"}</span>
                      <span className="ld-files-count">{"Fayllar soni"}: {answer?.file ? 1 : 0}</span>
                    </div>
                    {answer ? (
                      <>
                      ...
                      <div className="ld-block-body" style={{ color: "#999" }}>{"Hali jo'natma yo'q"}</div>
                    )}
                  </div>
                )}
                ...
                {result && (
                  <div className="ld-block">
                    <div className="ld-row-between">
                      <span className="ld-section-title">{"O'qituvchi izohi"}</span>
                      {status && (
                ...
                {canSubmit ? (
                  ...
                  <div className="ld-footer">{"Qayta topshirish imkoniyati berilmagan"}</div>
                )}
              </div>
            )}
          </div>
              ...
              <div key={l.id} className={`ld-lesson${isActive ? " active" : ""}`}>
                ...
                >
                  <div>
                    ...
                    <p className="ld-lesson-date">{"Dars sanasi"}: {fmtDate(l.created_at)}</p>
                  </div>
                  {hasVideos && (
